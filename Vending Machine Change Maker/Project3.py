...
def main():  # main code begins here, Chris Kau
    # initial stock
    n_s = 25
    d_s = 25
    q_s = 25
        ...
        if original_price < 0 or original_price % 5 != 0:  # if price entered is not valid
            ...
            while not_paid > 0:  # iterate through this loop until the amount paid >= the original price
                ...
                if deposit == 'n':
                    ...
                    not_paid -= 500
                    f_s += 1
                else:  # if the user enters 'c' for cancel: Kathryn Woest
                ...
                cents = not_paid % 100
            if not_paid < 0:  # if the user pays more than the original price, we need to calculate change: Kathryn Woest
                ...
                print("  No change due.")
                stock(n_s, d_s, q_s, o_s, f_s)
        # ask again for a new input, then loop to the top and check to see if it is 'q'.  if so, exit loop, Chris Kau
        price = input("Enter the purchase price (xx.xx) or 'q' to quit: ")
    stock_total(n_s, d_s, q_s, o_s, f_s)  # print the total stock once the user quits


if __name__ == "__main__":  # run the main code, Chris Kau
    main()
